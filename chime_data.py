...
def gen_flist_simu(chime_data_dir, stage, suffix_id):
    json_path = os.path.join(chime_data_dir, 'annotations', 
            '{}05_{}_{}.json'.format(stage, 'simu', suffix_id))
    with open(json_path, 'r') as fid:
        annotations = json.load(fid)
    seperated_dir = 'seperated'
    flist = [os.path.join(
            chime_data_dir, 'audio', '16kHz', seperated_dir,
            '{}05_{}_{}'.format(stage, a['environment'].lower(), 'simu'),
            '{}_{}_{}'.format(a['speaker'], a['wsj_name'], a['environment']))
             for a in annotations]
    return os.path.basename(json_path), flist


def gen_flist_real(chime_data_dir, stage, suffix_id):
    json_path = os.path.join(chime_data_dir, 'annotations',
            '{}05_{}_{}.json'.format(stage, 'real', suffix_id))
    with open(json_path, 'r') as fid:
        annotations = json.load(fid)
    flist_tuples = [(os.path.join(
                    chime_data_dir, 'audio', '16kHz', 'embedded', a['wavfile']),
                    a['start'], a['end'], a['wsj_name']) for a in annotations]
    return os.path.basename(json_path), flist_tuples
...
def prepare_training_data(chime_data_dir, dest_dir, suffix_id):
    for stage in ['tr', 'dt']:
        ...
        with open(os.path.join(dest_dir, 'flist_{}_{}.json'.format(stage, suffix_id)), 'w') as fid:
